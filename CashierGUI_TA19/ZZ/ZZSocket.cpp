...
BOOL CZZSocket::Connection(LPCSTR pstrHost, int nPort)
{
	if(m_bConnected) Close();
...
DWORD CALLBACK CZZSocket::TimeoutControl(LPVOID lpParm)
{
...
int CZZSocket::ReadData(char FAR* buf, int len, int timeout)
{
...
int CZZSocket::SendData(const char FAR *buf, int len, int timeout)
{
	if(!m_bConnected || m_sSocket == NULL)
	{
		err = -1;
...
BOOL CZZSocket::IsSockConnected(SOCKET s)
{
	int nRet = 0;
	/*
	因为还没有将该socket传入
...
BOOL CZZSocket::CreateServer(int port, int backlog)
{
...
SOCKET CZZSocket::Listen(char *ClientIP)
{
...
BOOL CZZSocket::StartServer(DEALPROC lpDealFunc)
{
	if(!m_bListened) return -1;
	if(m_sServer == NULL) return -1;
...
DWORD CALLBACK CZZSocket::ServerProc(LPVOID lpParm)
{
...
DWORD CALLBACK CZZSocket::DealProc(LPVOID lpParm)
{
...
SOCKET CZZSocket::GetSocket()
{
	return m_sSocket;
}

SOCKET CZZSocket::GetServerSocket()
{
	return m_sServer;
}

int CZZSocket::GetError()
{
	return err;
}